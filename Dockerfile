...
RUN echo "#!/bin/bash" > /entrypoint.sh && \
    ...
    echo "  echo \"\${BACKUP_SCHEDULE:-0 2 * * *} DB_HOST='\${DB_HOST}' DB_PORT='\${DB_PORT}' DB_USERNAME='\${DB_USERNAME}' DB_PASSWORD='\${DB_PASSWORD}' DB_DATABASE_NAME='\${DB_DATABASE_NAME}' UPLOAD_LOCATION='\${UPLOAD_LOCATION}' PBS_REPOSITORY='\${PBS_REPOSITORY}' PBS_PASSWORD='\${PBS_PASSWORD}' PBS_FINGERPRINT='\${PBS_FINGERPRINT}' PBS_NAMESPACE='\${PBS_NAMESPACE}' BACKUP_NAME='\${BACKUP_NAME}' /backup/scripts/backup.sh >> /proc/1/fd/1 2>&1\" | crontab -" >> /entrypoint.sh && \
